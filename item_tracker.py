...
class IsaacTracker:
    ...
    def load_options(self):
        ...
        if self.font:
            self.font = pygame.font.SysFont(self.options['show_font'], int(8 * self.options["size_multiplier"]),
                                            bold=self.options["bold_font"])
        self._image_library = {}
        self.roll_icon = pygame.transform.scale(self.get_image(self.id_to_image("284")), (
        int(16 * self.options["size_multiplier"]), int(16 * self.options["size_multiplier"])))
        self.blind_icon = pygame.transform.scale(self.get_image("collectibles/questionmark.png"), (
        int(16 * self.options["size_multiplier"]), int(16 * self.options["size_multiplier"])))
    ...
    def get_image(self, path):
        ...
        if image is None:
            ...
            scaled_image = pygame.transform.scale(image, (int(image.get_size()[0] * self.options["size_multiplier"]),
                                                          int(image.get_size()[1] * self.options["size_multiplier"])))
    ...
    def draw_floor(self, f, screen, my_font):
        pygame.draw.lines(
            ...
            self.color(self.options["text_color"]),
            False,
            ((f.x + 2, int(f.y + 24 * self.options["size_multiplier"])),
             (f.x + 2, f.y),
